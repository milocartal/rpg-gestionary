...
export default async function OG({ params }: Props) {
  const c = await db.population.findFirst({
    where: { slug: params.slug },
    select: { name: true, image: true, Universe: { select: { name: true } } },
  });

  const title = c?.name ?? "Population inconnue";
  const subtitle = c?.Universe.name ?? "Univers inconnu";
